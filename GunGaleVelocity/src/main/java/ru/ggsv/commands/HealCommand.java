...
public class HealCommand {
    ...
    public static BrigadierCommand createBrigadierCommand(final ProxyServer proxy) {
        LiteralCommandNode<CommandSource> helloNode = LiteralArgumentBuilder
                .<CommandSource>literal("heal")
                .executes(context -> {
                    if(!PlayerCommands.isUsed(context.getSource(), "heal")) {
                        ByteArrayOutputStream stream = new ByteArrayOutputStream();
                        DataOutputStream outputStream = new DataOutputStream(stream);

                        try {
                            String channel = "HealCmd";
                            String owner = "Console";
                            if (context.getSource() instanceof Player) {
                                owner = ((Player) context.getSource()).getUsername();
                                PlayerCommands.addUsedCommandInList(context.getSource(), "heal", 30);
                            }
